#!/usr/bin/env python3
"""
Script de prueba para validar el procesamiento de videos con Real-ESRGAN y Face Enhancement (GFPGAN).

Este script:
1. Lee un video de prueba (prueba_video_face.mp4)
2. Se autentica con el API
3. Envia el video para procesamiento con face_enhance=True
...
TEST_VIDEO = "prueba_video_face.mp4"
OUTPUT_DIR = "output"
...
TEST_MODEL = "general_x2"
TEST_SCALE = 2  # Escala 2 para que sea mas rapido
FACE_ENHANCE = True  # Activar mejora de rostros con GFPGAN
...
def wait_for_completion(token: str, video_id: str, max_wait_seconds: int = 7200) -> dict:
...
def main():
    print("=" * 70)
    print("Test de Procesamiento de Video con Real-ESRGAN + Face Enhancement")
    print("=" * 70)
    ...
    if not test_video_path.exists():
        ...
        print("Por favor, coloca un video llamado 'prueba_video_face.mp4' en la carpeta 'pruebas/'")
        sys.exit(1)
    ...
    print(f"  Face Enhancement: {FACE_ENHANCE}")

    try:
        start_time = time.time()
        result = enhance_video(token, video_base64, TEST_VIDEO, TEST_MODEL, TEST_SCALE, FACE_ENHANCE)
        ...
        print(f"  Face Enhance: {video_data.get('face_enhance', False)}")
    ...
    print(f"\n[5/5] Esperando procesamiento (con Face Enhancement esto tardara mas)...")
    try:
        ...
        if video_data.get("enhanced_base64"):
            ...
            output_enhanced = output_path / f"{video_name}_{TEST_MODEL}_face_enhanced.mkv"
    ...
    # Resumen
    print("\n" + "=" * 70)
    print("TEST COMPLETADO EXITOSAMENTE (con Face Enhancement)")
    print("=" * 70)
