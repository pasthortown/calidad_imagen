#!/usr/bin/env python3
"""
Script de prueba para validar todos los modelos de Real-ESRGAN disponibles en el API.

Este script:
1. Lee una imagen de prueba (prueba.png)
2. Se autentica con el API
3. Prueba cada modelo disponible
4. Guarda los resultados en ./output/ con el nombre del modelo aplicado
"""
...
def enhance_image(token: str, image_base64: str, model_type: str, filename: str, description: str = None) -> dict:
    """Envía una imagen al API para mejorarla con un modelo específico."""
    payload = {
...
def main():
    print("=" * 60)
    print("Test de Modelos Real-ESRGAN")
    print("=" * 60)
    ...
    for i, model_type in enumerate(available_models, 1):
        print(f"\n[{i}/{len(available_models)}] Procesando con modelo: {model_type}")

        try:
            start_time = time.time()

            # Llamar al API
            result = enhance_image(token, image_base64, model_type, TEST_IMAGE)

            elapsed = time.time() - start_time
            image_data = result["image"]

            # Guardar resultado
            output_filename = f"{image_name}_{model_type}.png"
    ...
    for r in results:
        if r["status"] == "OK":
            print(f"  {r['model']:15} | OK | {r['time_ms']:6}ms | {r['output_size']:12} | GPU: {r['gpu_used']}")
        else:
